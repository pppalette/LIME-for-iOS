 ...
 * File: Obfuscate.h
 * Project: SilentPwn
 * Author: LeFF && Batchh (Edited version)
 * Created: 2024-12-14
 *
 * Copyright (c) 2024 LeFF && Batchh. All rights reserved.
 *
 * Description: Obfuscation header for SilentPwn
 */
...
// Compile-time NSNumber encryption class for hex values
template <uintptr_t Value>
struct vxCplEncryptedHexNumber {
    uintptr_t EncryptedValue;

    // Compile-time constructor
    constexpr ALWAYS_INLINE vxCplEncryptedHexNumber()
    : EncryptedValue(Value ^ (seed)) {}

    // Run-time decryption
    inline uintptr_t decrypt() {
        return EncryptedValue ^ (seed);
    }
};

#ifdef __APPLE__
// Compile-time Objective-c string encryption macro
#define NSStringEncrypt(Str) @(ENCRYPT(Str))
// Compile-time NSNumber encryption macro for hex values
#define NSNumberEncrypt(Value) ([NSNumber numberWithUnsignedLong:vxCplEncryptedHexNumber<(uintptr_t)Value>().decrypt()])
#endif
...
// Compile-time NSNumber hex string encryption macro
#define IGSecretHexNumber(Str) ([NSNumber numberWithUnsignedLongLong:strtoull(ENCRYPT(Str), NULL, 0)])
